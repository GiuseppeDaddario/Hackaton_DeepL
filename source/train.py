...
def train_epoch(
    ...
    for batch_idx, data_batch in enumerate(tqdm(loader, desc=f"Epoch {current_epoch+1} Training", unit="batch", leave=False)):
        ...
        if effective_criterion_type == "ce" or effective_criterion_type == "ce_boost" or \
                effective_criterion_type == "sce" or effective_criterion_type == "gce":
            if effective_criterion_type == "ce_boost":
                loss_val = F.cross_entropy(output_logits, true_labels_int)
            else:
